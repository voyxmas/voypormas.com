...
class Eventos_ajax extends My_Controller
{
	...
	public function nuevo()
	{
		...
		$save['inscripciones_link'] = $this->input->post('inscripciones_link');
		...
		// crear el registro
		if (empty($e)) {
			$evento_id = $this->eventos_model->save($save);
			unset($save);
		} else
			$evento_id = false;
		...
		if (!empty($caracteristicas) and $evento_id) {
			...
			$caracteristicas_id = $this->eventos_caracteristicas_model->save($save);
			unset($save);
	...
	public function add_caracteristica($evento_id)
	{
		...
		if (!empty($caracteristica_id)) {
			...
			if (!empty($ya_existe)) {
				...
				$respuesta = $this->eventos_caracteristicas_model->save($attr);
				unset($attr);
	...
	public function agregar_variante($evento_id = null, $variante = null)
	{
		...
		$precios_id = $this->eventos_precios_model->save($save);
		unset($save);
