...
class Organizadores_ajax extends My_Controller
{
    ...
    public function identify()
    {
        ...
        if (!empty($this->input->post('email')) and !empty($this->input->post('password'))) {
            // actions
            $logincheck = $this->organizaciones_model->get($check);
            unset($check);
            ...
            if (!empty($logincheck)) {
                ...
                // todo: fix error cuando se intenta loguear con un usaurio borrado sin feedback al usuario
                // en este caso cuando un organizador fue borrado no encuentra el email, e intenta crear el registro, sql no puede tner el email duplicado y devuelve error
                $emailcheck = $this->organizaciones_model->get($check);
                unset($check);
                if (empty($emailcheck)) {
    ...
    public function aprobar($organizacion_id = null)
    {
        ...
        $attr['estado'] = 1;
        $respuesta = $this->organizaciones_model->save($attr, $organizacion_id);
        unset($attr);
